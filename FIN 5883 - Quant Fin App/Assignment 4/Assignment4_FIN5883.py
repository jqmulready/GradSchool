"""ticker symbol: ^SPX, maturity date: 2024-03-22"""
...
if(curve=='y'):
    f1='C:/Users/John Mulready/Desktop/John Mulready/Downloads/Spring 2024/FIN 5883 - Quant Fin App/bills2024-02-12.txt'
    f2='C:/Users/John Mulready/Desktop/John Mulready/Downloads/Spring 2024/FIN 5883 - Quant Fin App/bonds2024-02-12.txt'
...
#---------------------------------------------------------------------------
#Write a trading program that sells over valued (Based on IV curve), and buys undervalued (based on IV curve)

# Define your overvalued and undervalued thresholds for IV
overvalued_threshold = 0.15  # Adjust this threshold as needed
undervalued_threshold = 0.15  # Adjust this threshold as needed

# Check if IV is above the overvalued threshold
overvalued_options = calls[calls['IV'] > calls['IVfit']]

# Check if IV is below the undervalued threshold
undervalued_options = calls[calls['IV'] < calls['IVfit']]

# Plot the implied volatility curve
plt.plot(calls['Moneyness'], calls['IV'], color='black', label='Implied Volatility')

# Plot buy points
plt.scatter(undervalued_options['Moneyness'], undervalued_options['IV'], color='green', label='Buy')

# Plot sell points
plt.scatter(overvalued_options['Moneyness'], overvalued_options['IV'], color='red', label='Sell')

# Add labels and title
plt.xlabel('K/S')
plt.ylabel('Implied Volatility')
plt.title('Trading Signals based on Implied Volatility')
plt.legend()

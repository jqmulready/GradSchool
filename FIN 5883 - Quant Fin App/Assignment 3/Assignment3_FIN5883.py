...
def backtest_strategy_extended(prices, signals):
    ...
    plt.title('SMA Buy/Sell Signals')
    plt.legend()
    plt.show()
...
start_date = '2020-01-01'
end_date = datetime.now()
prices = fetch_data('PENN', '2020-01-01', end_date)
...
returns_df = yf.Ticker('PENN').history(period='5y', interval='1d').Close.to_frame()
...
plt.title('Bollinger Bands Buy/Sell Signal')
